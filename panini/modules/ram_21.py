...
a = second_word
output = a.replace(a[0], "")  # Replace the first character with an empty string
print(output)
final =first_word+output

print('final',final)
print('remove ्:',final[:-1])
import re

def merge_characters(word):
    merged_word = word.replace("्ओ", "ो").replace("्आ", "ा").replace("्अ", "").replace('श्इ','शि' ).replace('क्उ','कु')
    merged_word = re.sub(r'([इईउऊएऐऔअंअ:])्', r'\1', merged_word)
    return merged_word

# Merge characters in the string "ग्ओप्आल्अ"
#a = "गअम्अ्"
merged_string = merge_characters(final)
print(merged_string[:-1])  # Output: गोपाल
