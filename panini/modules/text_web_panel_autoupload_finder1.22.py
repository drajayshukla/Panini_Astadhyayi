...
all_output = ""  # Collect all output for copying
...
# Add a button to copy all content
if all_output:
    st.text_area("All Output", all_output, height=300)
    st.markdown(
        f'<button onclick="navigator.clipboard.writeText(`{all_output}`)">Copy All Content</button>',
        unsafe_allow_html=True
    )
