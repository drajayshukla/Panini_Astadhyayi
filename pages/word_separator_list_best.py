...
def main():
    ...
    input_string = st.text_area(
        "Enter the Devanagari text to process:",
        height=200  # Adjust height for larger inputs
    )

    # Process and display output if input is provided
    if st.button("Enter"):
        if input_string:
            # Limit input length for performance (optional)
            if len(input_string) > 10000:
                st.error("Input text is too long. Please limit it to 10,000 characters.")
            else:
                output_string = separate_characters_and_map(input_string, mapping)
                st.subheader("Mapped Output:")
                st.text_area("Output:", output_string, height=200)
        else:
