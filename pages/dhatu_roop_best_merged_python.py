...
DATA_FILE_PATH = '/Users/dr.ajayshukla/PycharmProjects/Panini_Astadhyayi/data/dhatupath/dhatupath reference/dhatupath_roop/merged.json'
...
def display_dhatu_set(set_name, data):
    print(f"# {set_name}")
    for dhatu_code, dhatu_data in data.items():
        display_dhatu(dhatu_code, dhatu_data)

def display_dhatu(dhatu_code, dhatu_data):
    print(f"### {dhatu_code} (कौमुदीधातुः)")
...
def search_dhatu(query, data):
    ...
    for set_name, dhatus in data.items():
        matching_dhatus = {key: value for key, value in dhatus.items() if query in key}
        if matching_dhatus:
            results[set_name] = matching_dhatus
    return results
...
def search_form(query, data):
    ...
    for set_name, dhatus in data.items():
        matching_dhatus = {}
        for dhatu_code, dhatu_data in dhatus.items():
...
if __name__ == "__main__":
    print("धातुरूप प्रदर्शनी")

    search_query = input("धातु कोड या रूप खोजें: ")
    if search_query:
        search_results = search_dhatu(search_query, DATA)
        form_results = search_form(search_query, DATA)

        if search_results:
            print("## कोड के लिए खोज परिणाम:")
        ...
        if form_results:
            print("## रूप के लिए खोज परिणाम:")
            for set_name, dhatus in form_results.items():
                print(f"### {set_name}")
                for code, details in dhatus.items():
                    print(f"#### धातु कोड: {code}")
        ...
        for set_name, dhatus in DATA.items():
            ...
            selected_dhatu = input(f"{set_name} से धातु कोड चुनें: ")
            if selected_dhatu in dhatus:
