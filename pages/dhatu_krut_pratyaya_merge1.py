...
# Function to search any Dhatu form
def search_dhatu_form(query, data):
    results = []
...
if data:
    ...
    all_dhatus = sorted(
        {standardize_dhatu_code(dhatu_code) for dhatus in data.values() for dhatu_code in dhatus.keys()})
    all_pratyayas = sorted(
        {pratyaya for dhatus in data.values() for pratyayas in dhatus.values() for pratyaya in pratyayas.keys()})

    dhatu_code = st.selectbox("Select a Dhatu Code:", all_dhatus)
    pratyaya = st.selectbox("Select a Pratyaya:", all_pratyayas)
    query = st.text_input("Search any Dhatu form:")

    # Search functionality
    if query:
        search_results = search_dhatu_form(query, data)
        if search_results:
            st.header("Search Results:")
            for result in search_results:
                file_name, code, pratyaya, forms = result
                st.write(f"**File:** {file_name}, **Dhatu Code:** {code}, **Pratyaya:** {pratyaya}, **Forms:** {forms}")
        else:
            st.warning("No matching Dhatu forms found.")
