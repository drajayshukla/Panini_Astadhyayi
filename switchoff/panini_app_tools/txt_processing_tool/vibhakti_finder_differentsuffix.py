...
#Removing redundancies in lists of strings.
#Excluding specific cases (entries starting with "ह्ए").
#Updating the data structure with new, relevant information ("suffix_only").
...
def remove_all_common_units(forms):
    ...
    updated_forms = []
    for form in forms:
        if not form.startswith("ह्ए"):  # Only modify forms not starting with 'ह्ए'
            updated_forms.append(form.replace(common_substring, ""))
        else:
            updated_forms.append(form)  # Keep 'ह्ए' forms unchanged
